...
const Chat = ({
    ...
    const IsShowRating =
        activeTab === 'buying' && selectedTabData?.item?.status === 'sold out'
    const IsShowMakeOfferOnTop =
        IsTopMakeOffer &&
        chatMessages?.length > 0 &&
        !selectedTabData?.amount &&
        !makeUnableToChat &&
        activeTab === 'buying'
    const IsShowInitialMakeOffer =
        chatMessages?.length === 0 &&
        !selectedTabData?.amount &&
        !IsMakeOfferInChat &&
        activeTab === 'buying' &&
        !makeUnableToChat
    const IsShowOfferInput =
        IsMakeOfferInChat &&
        activeTab === 'buying' &&
        !selectedTabData?.amount &&
        !makeUnableToChat
    ...
    useEffect(() => {
        ...
        if (
            activeTab === 'selling' &&
            globalNotifications?.tab === 'selling' &&
            Number(selectedTabData?.id) !== Number(globalNotifications?.id)
        ) {
                ...
                prevList.map((chat) =>
                    chat?.id === Number(globalNotifications?.id)
                        ? {
                              ...chat,
                              unread_chat_count:
                                  globalNotifications?.unread_chat_count,
                              created_at: globalNotifications?.created_at,
                          }
                        : chat
                )
            )
            ...
            activeTab === 'buying' &&
            globalNotifications?.tab === 'buying' &&
            Number(selectedTabData?.id) !== Number(globalNotifications?.id)
        ) {
        ...
        try {
            ...
            if (response?.data?.error === false) {
                ...
                setBuyerChatList((prev) =>
                    prev.map((item) => {
                        if (
                            Number(item.id) === Number(response?.data?.data?.id)
                        ) {
    ...
    const handleReviewSubmit = async () => {
        ...
        try {
            const res = await addItemReviewApi.addItemReview({
                item_id: selectedTabData?.item_id,
                review: Review,
                ratings: Rating,
            })
            ...
            if (error) {
                switch (error) {
                    case 'permission_denied':
                        toast.error(
                            'Microphone access denied. Enable permissions.'
                        )
                        break
    ...
    return (
        <>
                                            ...
                                            <div className='chatRatingWrap'>
                                                {IsShowMakeOfferOnTop && (
                                                    <div className='topMakeOffer'>
                                                        <h1 className='submitProposal'>
                                                            {t(
                                                                'submitProposal'
                                                            )}
                                                        </h1>
                                                        <button
                                                            className='makeOfferCont'
                                                            onClick={
                                                                handleTopMakeOfferClick
                                                            }
                                                        >
                                                            <span>
                                                                <IoGiftOutline
                                                    ...
                                                    !selectedTabData?.item
                                                        ...
                                                        <div className='rateSellerWrap'>
                                                            <div className='rateSellerCont'>
                                                                <h6 className='rateSellerHeading'>
                                                                    {t(
                                                                        'rateSeller'
                                                                    )}
                                                                </h6>
                                                                <div className='rateExpCont'>
                                                                    <p className='rateYourExp'>
                                                                        {t(
                                                                            'rateYourExp'
                                                                        )}
                                                                    </p>
                                                                    ...
                                                                    <textarea
                                                                        ...
                                                                        className='auth_input no-resize'
                                                                        id=''
                                                                        cols='30'
                                                                        rows='5'
                                                                        placeholder={t(
                                            ...
                                            {makeUnableToChat ? (
                                                ...
                                                >
                                                    ...
                                                    <div className='inputs'>
                                                        {isRecording ? (
                                                            <div
                                                                className='close_icon_cont cursor-pointer'
                                                                onClick={
            ...
            <TipsModal
                ...
                OnHide={() => setIsTipsOpen(false)}
                setIsMakeOfferInChat={setIsMakeOfferInChat}
                tipsData={tipsData}
                setIsTopMakeOffer={setIsTopMakeOffer}
            />
        </>
    )
}
