...
import paytr from '../../../../public/assets/paytr_logo.png'
    ...
    const handlePaytrPayment = async () => {
        ...
        try {
            const res = await createPaymentIntentApi.createIntent({
                package_id: priceData.id,
                payment_method: PaytrActive.payment_method,
                platform_type: 'web'
            });
            if (res.data.error) {
                toast.error(res.data.message);
            ...
            const payment_gateway_response = res.data.data.payment_intent.iframe_url;
                                    ...
                                    {PaytrActive?.status === 1 &&
                                        <div className="col-12">
                                            <button onClick={handlePaytrPayment}>
                                                <div className="payment_details">
                                                    <Image loading='lazy' src={paytr} onEmptiedCapture={placeholderImage} width={30} height={30} />
                                                    <span>
                                                        {t('paytr')}
                                                    </span>
                                                </div>
