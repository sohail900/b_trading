...
const ProfileSubscription = () => {
    ...
    useEffect(() => {
        if (AdListingRef && AdListingRef?.current) {
            AdListingRef?.current?.changeLanguageDirection(
                isRtl ? 'rtl' : 'ltr'
            )
        }
    }, [isRtl])
    useEffect(() => {
        if (FeaturedAdRef && FeaturedAdRef?.current) {
            FeaturedAdRef?.current?.changeLanguageDirection(
                isRtl ? 'rtl' : 'ltr'
            )
        }
    }, [isRtl])
                                    ...
                                    {advertisementPackage?.length > 0 && (
                                        <div className='sub_content'>
                                            ...
                                            <Swiper
                                                ...
                                                modules={[FreeMode]}
                                                onSwiper={(swiper) => {
                                                    FeaturedAdRef.current =
                                                        swiper
                                                }}
                                                dir={isRtl ? 'rtl' : 'ltr'}
                                            >
                                                {advertisementPackage?.map(
                                                    (data, index) => (
                                                        <SwiperSlide
                                                            key={index}
                                                        >
